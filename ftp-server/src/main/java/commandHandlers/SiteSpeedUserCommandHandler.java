...
public class SiteSpeedUserCommandHandler extends BaseCommandHandler {
    ...
    protected FtpResponse executeCommand(String arguments, User user) {
        ...
        try {
            ...
            if (userSpeedLimit == 0) {
                boolean success = ftpServer.getController().resetUserSpeedLimit(targetUsername);
                if (success) {
                    return new FtpResponse(200, "Speed limit for user " + targetUsername + " has been reset");
                } else {
                    return new FtpResponse(550, "Failed to reset speed limit. User not found or error occurred");
                }
            }

            if (userSpeedLimit <= 0) {
                return new FtpResponse(501, "Speed limit must be a positive integer or zero");
            }
